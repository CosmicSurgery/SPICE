...
def scan_raster(T_labels, N_labels, window_dim = None):
    ...
    start = time.time()
    Z_BY_NEURON = _build_dendrograms(sim_mats)
    for i, cutoff in enumerate(list_cutoffs): 
        clusters = _cluster_windows(cutoff, N_labels, sim_mats, Z_by_neuron=Z_BY_NEURON)
...
def _build_dendrograms(sim_mats):
    """
    Precompute a complete-linkage dendrogram once per neuron from its
    (already-built) similarity/distance matrix in sim_mats[n].
    Returns: dict neuron_id -> Z (or None if not applicable).
    """
    Z_by_neuron = {}
    for n, M in sim_mats.items():
        if isinstance(M, np.ndarray) and M.ndim == 2 and M.shape[0] > 1 and not np.all(M == 0):
            Z_by_neuron[n] = linkage(M, method='complete')  # exactly what you do per-cutoff today
        else:
            Z_by_neuron[n] = None
    return Z_by_neuron
...
def _cluster_windows(cutoff, N_labels, sim_mats, Z_by_neuron=None):
    clusters = np.zeros_like(N_labels, dtype=int)
    label_offset = 0
    for n in np.unique(N_labels):
        idxs = np.where(N_labels == n)[0]
        if idxs.size == 0:
            continue
        if Z_by_neuron is not None:
            Z = Z_by_neuron.get(n, None)
            if Z is None:
                # single window or empty matrix → leave zeros (one cluster)
                continue
            lab = fcluster(Z, t=cutoff, criterion='distance')
        else:
            # Fallback to original per-cutoff behavior (recompute linkage here)
            M = sim_mats[n]
            if not (isinstance(M, np.ndarray) and M.ndim == 2 and M.shape[0] > 1 and not np.all(M == 0)):
                continue
            Z = linkage(M, method='complete')
            lab = fcluster(Z, t=cutoff, criterion='distance')
        # Make labels globally unique across neurons
        clusters[idxs] = label_offset + lab
        label_offset = int(np.max(clusters))
    return clusters
